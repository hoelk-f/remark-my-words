...
export async function testCategories({evaluate, until, cdp}) {
  ...
  assert.equal(await evaluate('document.querySelector("#pdfaw-accent-color").value'),'#55d8c8');
  ...
  await evaluate(`(() => {const color=document.querySelector('#pdfaw-accent-color');color.value='#b54cff';color.dispatchEvent(new Event('input'));})()`);
  ...
  assert.equal(await evaluate('pluginData.accentColor'),'#b54cff');
